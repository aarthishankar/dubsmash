...
//echo rand(1,155);
 ...
 // $profileID=$id_prof;
   ...
   //$select_movie="select * from dubs_contestor where movieID='$movie_id'";
  //$select_movie=" select * from dubs_movie_details where `MovieTags`  REGEXP 'comedia|action|chara' limit 5 ";
  
  // to take posted by
/*   $select_movie="SELECT competitorID FROM  `dubs_contestmovie` where movieID='$movie_id' ";
    $result_select_movie=mysql_query($select_movie);
   $row_select_movie=mysql_fetch_array($result_select_movie);
   $competit=$row_select_movie['competitorID'];
   
   $post_query="SELECT * FROM dubs_contestor where competitorID='$competit'";
    $post_result=mysql_query($post_query);
   $post_row=mysql_fetch_array($post_result);
   $contestname=$post_row['contest_name'];
   $contestID=$post_row['contest_name'];*/
