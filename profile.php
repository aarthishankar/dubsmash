...
if(!empty($str)) {
	...
	if(isset($_SESSION['u_id'])) {
		...
		$query2 = mysqli_query($con, $check_contest);
		$rows = mysqli_num_rows($query2);
		if ($rows == 1) { 
			$ro=mysqli_fetch_array($query2);
			$id_prof=$ro['competitorID'];
			$_SESSION['contID']=$id_prof;
		}else{
		
	echo "<script>
    window.location = 'ajax/acceptance.php';
</script>";
		}
	}else{
	echo "<script>
...
if(isset($_SESSION['u_id'])){
	$u_id=$_SESSION['u_id'];
	$check_ownership="SELECT * FROM dubs_contestor where littleID='$u_id' and competitorID='$id_prof' ";
	$result_check_ownership=mysqli_query($con,$check_ownership);
	$v_result_check_ownership=mysqli_num_rows($result_check_ownership);
...
$result_detail=mysqli_query($con,$take_details);
$row_details=mysqli_fetch_array($result_detail);
